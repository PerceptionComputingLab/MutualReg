    ...
    with torch.enable_grad():
        ...
        for iter in range(50):
            ...
            disp_sample = F.avg_pool3d(
                F.avg_pool3d(F.avg_pool3d(net[0].weight, 3, stride=1, padding=1), 3, stride=1, padding=1), 3, stride=1, padding=1)
            ...
            grid_disp = grid_disp.view(1, H // grid_sp, W // grid_sp, D // grid_sp, 3).cuda()
            patch_mov_sampled = F.grid_sample(mind_mov.cuda().float(),
                                              grid_disp, align_corners=False, mode='bilinear')
