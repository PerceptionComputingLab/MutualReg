...
from registration_pipeline import update_fields, update_fields_multKD_DB_teach
...
def train(args):
    ...
    tag = '/a_teach/AbdCT_teach_lr_{}_smh_{}_iter{}_Tag_1.11'.format(lr, smooth, half_iterations)
    out_dir = out_dir + tag
    print('tag: ', tag)
    ...
    feature_net_teacher = nn.Sequential(nn.Conv3d(1, 32, 3, padding=1, stride=2), nn.BatchNorm3d(32), nn.ReLU(),
    ...
    for i in range(half_iterations):
        ...
        if i % 1000 == 999:
            ...
            if use_adam:
                ...
                if (d_all_net_teacher_test.sum() / (d_all_ident_teacher_test > 0.1).sum()).item() > best_DSC_tea_test:
                    best_DSC_tea_test = (d_all_net_teacher_test.sum() / (d_all_ident_teacher_test > 0.1).sum()).item()
