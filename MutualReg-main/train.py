...
def train(args):
    ...
    for repeat in range(2):
        ...
        eta_min = 0.00001
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 50, 1, eta_min=eta_min)
            ...
            for i in range(half_iterations):
                    ...
                    if do_augment:
                        ...
                        with torch.no_grad():
                            for j in range(len(idx)):
                                ...
                                img1[j:j + 1] = F.grid_sample(img1_[j:j + 1], affine2[j:j + 1])
                                target[j:j + 1] = disp_field_aff
                    img0 = torch.clamp(img0, -.4, .6)  # todo:增加clamp，测试效果
                    img1 = torch.clamp(img1, -.4, .6)
                ...
                if ((i % 1000 == 999)):
                    ...
                    if use_adam:
                        ...
                        print('fields updated val error :', d_all0[:3].mean(), '>', d_all_net[:3].mean(), '>',
                              d_all_adam[:3].mean())
                        print('d_all0: ', d_all0.size(), d_all0.mean(), d_all0)
                        print('d_all_net: ', d_all_net.size(), d_all_net.mean(), d_all_net)
                        print('d_all_adam: ', d_all_adam.size(), d_all_adam.mean(), d_all_adam)
