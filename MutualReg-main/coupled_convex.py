...
def coupled_convex(feat_fix, feat_mov, use_ice, img_shape):
    disp_hw = 2
    ssd = correlate(5 * (feat_fix), 5 * (feat_mov), disp_hw)  # shape:torch.Size([1, 125, 24, 20, 32])
    disp_mesh_t = F.affine_grid(disp_hw * torch.eye(3, 4).cuda().unsqueeze(0),
                                (1, 1, disp_hw * 2 + 1, disp_hw * 2 + 1, disp_hw * 2 + 1),
                                align_corners=True).permute(0, 4, 1, 2, 3).reshape(3, -1, 1)  # torch.Size([3, 125, 1])
    H2, W2, D2 = ssd.shape[-3:]
    disp_soft0 = coupled_convex_optim(ssd, disp_mesh_t)  # torch.Size([1, 3, 24, 20, 32])

    if use_ice:
        # print('use_ice: ', use_ice)
    ...
    scale = torch.tensor([H2 - 1, W2 - 1, D2 - 1]).view(1, 3, 1, 1, 1).cuda() / 2  # torch.Size([1, 3, 1, 1, 1])
    disp_torch = F.avg_pool3d(
        F.avg_pool3d(F.interpolate((disp_soft0 / scale).flip(1), scale_factor=2, mode='trilinear'), 3,
                     padding=1, stride=1), 3, padding=1, stride=1)  # torch.Size([1, 3, 48, 40, 64])

    disp_soft1 = F.interpolate(disp_torch, scale_factor=2, mode='trilinear', align_corners=False,
                               recompute_scale_factor=False)  # torch.Size([1, 3, 96, 80, 128])
    disp_soft2 = F.avg_pool3d(F.avg_pool3d(disp_soft1, 3, stride=1, padding=1), 3, stride=1, padding=1) # torch.Size([1, 3, 96, 80, 128])
    disp_soft3 = F.interpolate(disp_soft2, size=img_shape, mode='trilinear')  # torch.Size([1, 3, 192, 160, 256])

    return disp_soft3

def coupled_convex_training(feat_fix, feat_mov, use_ice, img_shape):
    disp_hw = 2
    ssd = correlate(5 * (feat_fix), 5 * (feat_mov), disp_hw)  # shape:torch.Size([1, 125, 24, 20, 32])
    disp_mesh_t = F.affine_grid(disp_hw * torch.eye(3, 4).cuda().unsqueeze(0),
                                (1, 1, disp_hw * 2 + 1, disp_hw * 2 + 1, disp_hw * 2 + 1),
    ...
    if use_ice:
        ...
        disp_soft0_ = coupled_convex_optim(ssd_, disp_mesh_t)
        scale = torch.tensor([H2 - 1, W2 - 1, D2 - 1]).view(1, 3, 1, 1, 1).cuda() / 2
        disp_soft0_temp = inverse_consistency((disp_soft0 / scale).flip(1), (disp_soft0_ / scale).flip(1), iter=15)[0].flip(1) * scale

    scale = torch.tensor([H2 - 1, W2 - 1, D2 - 1]).view(1, 3, 1, 1, 1).cuda() / 2  # torch.Size([1, 3, 1, 1, 1])
    disp_torch = F.avg_pool3d(
        F.avg_pool3d(F.interpolate((disp_soft0_temp / scale).flip(1), scale_factor=2, mode='trilinear'), 3,
                     padding=1, stride=1), 3, padding=1, stride=1)  # torch.Size([1, 3, 48, 40, 64])
