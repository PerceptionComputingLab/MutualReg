...
def train(args):
    ...
    tag = '/a_t2s2t2s_finetune/AbdCT_t2s2t2s_ft_mask_lr_{}_smh_{}_flow_{}_iter{}_5.11'.format(lr, smooth, flow, half_iterations)
    out_dir = out_dir + tag
    print('tag: ', tag)
    ...
    model_dir = './results/multKD/a_t2s2t_finetune/AbdCT_t2s2t_ft_mask_lr_0.001_smh_0_flow_10_iter30000_4.11/'
    model_idx = -2
    ...
    print('Using Teacher model for producing "Flow GT": use_adam=True, num_warps=2, ice=True reg_fac=10')
    ...
    logJ_net_student, logJ_adam_student, Jacob_net_student, Jacob_adam_student = \
        update_fields_multKD_DB_student(data, best_model_student, use_adam=True, num_warps=2, ice=True,
                                        reg_fac=3, compute_jacobian=True)
